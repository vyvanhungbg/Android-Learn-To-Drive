...
private const val TAG = "QuestionDieViewModel"

class QuestionDieViewModel(
    ...
    init {
     ...
     fun getAllQuestionsDie() {
        registerDisposable(
            executeTask(
                task = {   questionRepository.getQuestionDieByLicense(sharedPreferencesSettings.getCurrentLicenseType());
                     },
                onSuccess = {
                    _questions.value = it.map { item -> item.toQuestion() }
